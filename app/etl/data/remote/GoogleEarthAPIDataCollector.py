import ee
import pandas as pd
from app.etl.data.remote.DataProcessor import DataProcessor
...
class GoogleEarthAPIDataCollector :
    ...
    def collect(self,start_date,end_date,longitude,latitude,scale):
        ...
        df = self.__load_data_from_dataset(dataset,start_date,end_date,longitude,latitude,scale)
        weather_df= self.__process_data(df)
        return weather_df
    def __process_data(self,df):
        calculator_instance = DataProcessor()
        weather_df = pd.DataFrame()
        weather_df['date'] = df['time'].apply(lambda x: pd.to_datetime(x / 1000, unit="s"))
        weather_df['temperature'] = df['temperature_2m'] - 273.15
        weather_df['soil_temperature'] = df['soil_temperature_level_1'] - 273.15


        # m/s meters per second
        weather_df['wind_speed'] = calculator_instance.calculate_wind_speed(df['u_component_of_wind_10m'], df['v_component_of_wind_10m'])

        # Degree In 360
        weather_df['wind_direction'] = calculator_instance.calculate_wind_direction(df['u_component_of_wind_10m'],
        ...
        weather_df['relative_humidity'] = calculator_instance.calculate_relative_humidity(df['temperature_2m'],
                                                                      df['dewpoint_temperature_2m'])



        # grams of water vapor per kilogram of moist air (g/kg).
        weather_df['specific_humidity'] = calculator_instance.calculate_specific_humidity(df['dewpoint_temperature_2m']
                                                                      , df['surface_pressure'])
        ...
        weather_df["evaporation"] = df["total_evaporation_sum"] - df["evaporation_from_vegetation_transpiration_sum"]
        weather_df = weather_df.round(5)
        return weather_df
    def __load_data_from_dataset(self, dataset, start_date, end_date, latitude, longitude,scale):
        point = ee.Geometry.Point([longitude, latitude])

        dataset = ee.ImageCollection(dataset).filterDate(start_date, end_date)
        dataset = dataset.select(['temperature_2m', 'soil_temperature_level_1', 'u_component_of_wind_10m'
